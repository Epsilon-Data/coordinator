...
| Metric | Before (Base64 JSON) | After (Binary Format) | Improvement |
|--------|---------------------|------------------------|-------------|
| Bundle Size | 1GB+ | ~45KB | 2000%+ reduction |
| Processing Time | 30+ seconds | <2 seconds | 15x faster |
| Memory Usage | High (OOM errors) | Minimal | Stable execution |
| Storage Efficiency | 300% overhead | 10% overhead | 30x more efficient |

### Security Properties

- **Data at Rest**: All CSV files encrypted with AES-256
- **Key Management**: AWS KMS handles all cryptographic keys
- **Zero Knowledge**: Host never sees plaintext data
- **Temporary Decryption**: CSV only decrypted in secure enclave
- **Audit Trail**: All KMS operations logged and auditable

## Components

### Dataset Manager (`workers/executor/services/dataset_manager.py`)
- Handles CSV file encryption and bundle preparation
- Implements binary encryption format for optimal size
- Integrates with AWS KMS for secure key management

### Enclave Client (`workers/executor/enclave/enclave_client_local.py`)
- Manages script execution in secure environments
- Handles CSV decryption within trusted boundaries
- Supports both local testing and production Nitro Enclaves

### CSV Decryptor (`workers/executor/enclave_app/csv_decryptor.py`)
- Dedicated component for CSV file decryption
- Supports both legacy JSON and new binary formats
...
- AWS CLI configured with appropriate KMS permissions
- Python 3.11+
...
# Execution Mode
USE_LOCAL_ENCLAVE=true  # Set to false for production Nitro Enclaves
...
1. **Code Changes**: Edit source files in `workers/` directories
2. **Rebuild**: `docker-compose build executor-worker`
3. **Restart**: `docker-compose restart executor-worker`
4. **Test**: Submit jobs via API and monitor logs

## Security Model

### Threat Model
- **Untrusted Host**: Host infrastructure may be compromised
- **Trusted Enclave**: Nitro Enclave provides hardware-level isolation
- **Encrypted Transit**: All data encrypted in transit via TLS
- **Encrypted Storage**: All sensitive data encrypted at rest

### Security Guarantees
- **Confidentiality**: Data only decrypted within secure enclaves
- **Integrity**: Cryptographic verification of all data transfers
- **Auditability**: Complete audit trail via AWS CloudTrail
...
- **GDPR**: Data privacy through secure processing and minimal exposure
...
- **Symptoms**: Memory errors, executor crashes
- **Cause**: Old Base64 JSON encryption format
- **Solution**: System automatically uses binary format for new jobs

#### CSV File Not Found
- **Symptoms**: "CSV file not found" in script output
- **Cause**: Decryption failure or missing archetype directory
- **Debug**: Check logs for "🔓 Decrypting CSV" messages

#### KMS Permission Denied
- **Symptoms**: "AccessDenied" errors in logs
- **Cause**: Insufficient AWS KMS permissions
- **Solution**: Ensure KMS key policy allows encrypt/decrypt operations

### Debug Logging

Enable detailed CSV processing logs:
```bash
# View encryption logs
docker-compose logs -f executor-worker | grep "📊\|🔐\|✅"
